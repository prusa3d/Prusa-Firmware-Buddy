...
void test_resp_req(const char *server_resp, Status status, ContentType content_type, const char *body) {
...
TEST_CASE("Request - response no content") {
    ...
    test_resp_req(mock_resp, Status::NoContent, ContentType::ApplicationOctetStream, "");
}

TEST_CASE("Request - response with body") {
    test_resp_req(mock_resp_body, Status::Ok, ContentType::ApplicationJson, "Hello world");
}
