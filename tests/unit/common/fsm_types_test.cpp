...
public:
    ...
    void TestEmpty() {
        REQUIRE(GetCount() == 0);
        REQUIRE_FALSE(prior_command_in_queue0);
...
TEST_CASE("fsm::SmartQueue", "[fsm]") {
    ...
    SECTION("Wrong destroy") {
        ...
        std::optional<DequeStates> states = q.dequeue(); // queue should not send anything
    ...
    SECTION("Multiple destroy") {
        ...
        states = q.dequeue();
        REQUIRE(states.has_value()); // dequeuing first destroy
        REQUIRE(states->current.get_fsm_type() == ClientFSM::_none);
        REQUIRE(states->current.get_queue_index() == QueueIndex::q0);
        REQUIRE(states->last_sent.get_fsm_type() == ClientFSM::Load_unload);
        REQUIRE(states->last_sent.get_queue_index() == QueueIndex::q0);
        q.TestEmpty();
    }
    ...
    SECTION("Multiple change") {
        ...
        REQUIRE(q.GetCount() == 0); // insertion failed
        REQUIRE(q.GetOpenFsmQ0() == ClientFSM::Preheat);
        REQUIRE(q.GetOpenFsmQ1() == ClientFSM::_none);
    ...
    SECTION("Max queue length") {
        ...
        //push create to q0
        const BaseData data2(0x25, { { 0x02, 0x00, 0x00, 0x00 } });
        REQUIRE(q.PushCreate(ClientFSM::Preheat, data) == SmartQueue::Selector::q0);
        REQUIRE(q.GetCount() == 1);
        REQUIRE(q.GetOpenFsmQ0() == ClientFSM::Preheat);
        REQUIRE(q.GetOpenFsmQ1() == ClientFSM::_none);
        ...
        const BaseData data4(0xAB, { { 0x02, 0x00, 0x00, 0x00 } });
        REQUIRE(q.PushCreate(ClientFSM::CrashRecovery, data4) == SmartQueue::Selector::error);
        REQUIRE(q.GetCount() == 2);
        REQUIRE(q.GetOpenFsmQ0() == ClientFSM::Preheat); //new one (but same as old one)
        REQUIRE(q.GetOpenFsmQ1() == ClientFSM::Load_unload);
