...
*/
#include "IWatchdog.h"
#include "../../cores/arduino/stm32/LL/stm32yyxx_ll_iwdg.h"
#include "../../cores/arduino/stm32/LL/stm32yyxx_ll_rcc.h"
...
#ifdef IWDG_WINR_WIN
    if ((window != IWDG_TIMEOUT_MAX) && (LL_IWDG_GetWindow(IWDG) != IWDG_WINR_WIN)) {
        if (window >= timeout) {
            // Reset window value
