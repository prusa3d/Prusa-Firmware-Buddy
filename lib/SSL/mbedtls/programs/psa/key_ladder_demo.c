...
#if !defined(MBEDTLS_SHA256_C) || !defined(MBEDTLS_MD_C) || !defined(MBEDTLS_AES_C) || !defined(MBEDTLS_CCM_C) || !defined(MBEDTLS_PSA_CRYPTO_C) || !defined(MBEDTLS_FS_IO) || defined(MBEDTLS_PSA_CRYPTO_KEY_ID_ENCODES_OWNER)
int main(void) {
...
static psa_status_t unwrap_data(const char *input_file_name,
    ...
    ciphertext_size = PSA_AEAD_ENCRYPT_OUTPUT_SIZE(WRAPPING_ALG, header.payload_size);
    /* Check for integer overflow. */
