...
int main(int argc, char *argv[]) {
    ...
    if (opt.force_ciphersuite[0] > 0) {
        ...
        if (opt.psk_opaque != 0 || opt.psk_list_opaque != 0) {
            ...
            if (ciphersuite_info->key_exchange != MBEDTLS_KEY_EXCHANGE_PSK || opt.min_version != MBEDTLS_SSL_MINOR_VERSION_3) {
                mbedtls_printf("opaque PSKs are only supported in conjunction with forcing TLS 1.2 and a PSK-only ciphersuite through the 'force_ciphersuite' option.\n");
                ret = 2;
                goto usage;
            }
    ...
    if (mbedtls_test_unhexify(cid_renego, sizeof(cid_renego),
            opt.cid_val_renego, &cid_renego_len)
        != 0) {
        mbedtls_printf("CID not valid hex\n");
        goto exit;
    }
    ...
    if (opt.use_srtp == 1) {
        ...
        mbedtls_ssl_conf_srtp_mki_value_supported(&conf,
            opt.support_mki ? MBEDTLS_SSL_DTLS_SRTP_MKI_SUPPORTED : MBEDTLS_SSL_DTLS_SRTP_MKI_UNSUPPORTED);
...
exit:
    ...
    #if defined(MBEDTLS_KEY_EXCHANGE_SOME_PSK_ENABLED) && defined(MBEDTLS_USE_PSA_CRYPTO)
    if (opt.psk_opaque != 0) {
