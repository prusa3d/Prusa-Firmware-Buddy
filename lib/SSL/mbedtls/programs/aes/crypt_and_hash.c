...
int main(int argc, char *argv[]) {
    ...
    if ((fkey = fopen(argv[6], "rb")) != NULL) {
        ...
        if (memcmp(argv[6], "hex:", 4) == 0) {
            ...
            while (sscanf(p, "%02X", (unsigned int *)&n) > 0 && keylen < (int)sizeof(key)) {
    ...
    if (mode == MODE_DECRYPT) {
         ...
         * Check the file size.
         */
        if (cipher_info->mode != MBEDTLS_MODE_GCM && ((filesize - mbedtls_md_get_size(md_info)) % mbedtls_cipher_get_block_size(&cipher_ctx)) != 0) {
