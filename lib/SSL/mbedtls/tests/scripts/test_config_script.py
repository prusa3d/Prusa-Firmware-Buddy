...
def output_file_name(directory, stem, extension):
    return os.path.join(directory, '{}{}.{}'.format(OUTPUT_FILE_PREFIX, stem,
                                                    extension))
...
def guess_presets_from_help(help_text):
    ...
    if hits:
        return hits
    raise Exception(
        "Unable to figure out supported presets. Pass the '-p' option.")
...
def run_one(options, args, stem_prefix='', input_file=None):
    ...
    * config-xxx.status: exit code.
    ...
    status_filename = output_file_name(options.output_directory, stem,
                                       'status')
    ...
    cmd = [
        os.path.abspath(options.script), '-f',
