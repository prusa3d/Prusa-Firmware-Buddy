...
class read_file_lines:
    ...
    def __enter__(self):
        self.generator = enumerate(
            open(self.filename, 'rb' if self.binary else 'r'))
        return self
...
class Inputs:
    ...
    def __init__(self):
        ...
        self.hash_algorithms = set(['0x020000fe'
                                    ])  # 0x020000ff is PSA_ALG_ANY_HASH
...
class Tests:
    ...
    def run_one(self, inputs, type_word):
        ...
        take arguments.
        """
        expressions, values = collect_values(inputs,
                                             type_word,
        ...
        for expr, value, output in zip(expressions, values, outputs):
            if self.options.show:
                sys.stdout.write('{} {}\t{}\n'.format(type_word, value,
                                                      output))
            if normalize(expr) != normalize(output):
                self.errors.append(
                    self.Error(type=type_word,
                               expression=expr,
                               value=value,
                               output=output))
    ...
    def run_all(self, inputs):
        ...
        for type_word in [
                'status', 'algorithm', 'ecc_curve', 'dh_group', 'key_type',
                'key_usage'
        ]:
