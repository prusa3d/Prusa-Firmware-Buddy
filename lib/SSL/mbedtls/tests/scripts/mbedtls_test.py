...
class TestDataParser:
    ...
    def __escaped_split(inp_str, split_char):
        ...
        out = list(
            map(split_colon_fn, re.split(r'(?<!\\)' + split_char, inp_str)))
    ...
    def __parse(self, data_f):
        ...
        for line in data_f:
            ...
            self.tests.append(
                (name, function_name, dependencies, grouped_args))
...
class MbedTlsTest(BaseHostTest):
    ...
    def setup(self):
        ...
        data_file = os.path.join(script_dir, '..', 'mbedtls', suite_name,
                                 data_file)
    ...
    def run_test(self, name, function_id, dependencies, args):
        ...
        param_bytes, length = self.test_vector_to_bytes(
            function_id, dependencies, args)
        self.send_kv(''.join('{:02x}'.format(x) for x in length),
    ...
    def on_result(self, _key, value, _timestamp):
        ...
        self.log('{{__testcase_finish;%s;%d;%d}}' %
                 (name, int_val == 0, int_val != 0))
        if int_val != 0:
