...
def psa_want_symbol(name: str) -> str:
    ...
    if name.startswith('PSA_'):
        ...
        raise ValueError('Unable to determine the PSA_WANT_ symbol for ' +
                         name)
...
def finish_family_dependencies(dependencies: List[str],
                               bits: int) -> List[str]:
...
def read_implemented_dependencies(filename: str) -> FrozenSet[str]:
    return frozenset(symbol for line in open(filename)
                     for symbol in re.findall(r'\bPSA_WANT_\w+\b', line))


IMPLEMENTED_DEPENDENCIES = read_implemented_dependencies(
    'include/psa/crypto_config.h')


def hack_dependencies_not_implemented(dependencies: List[str]) -> None:
    if not all(
            dep.lstrip('!') in IMPLEMENTED_DEPENDENCIES
            for dep in dependencies):
        dependencies.append('DEPENDENCY_NOT_IMPLEMENTED_YET')


def test_case_for_key_type_not_supported(verb: str,
                                         key_type: str,
                                         bits: int,
    ...
    tc.set_description('PSA {} {} {}-bit {} supported'.format(
        verb, short_key_type, bits, adverb))
...
class TestGenerator:
    ...
    @staticmethod
    def remove_unwanted_macros(constructors: macro_collector.PSAMacroCollector
                               ) -> None:
        ...
        constructors.algorithms_from_hash.pop('PSA_ALG_DETERMINISTIC_DSA',
                                              None)
    ...
    def generate_not_supported(self) -> None:
        ...
        self.write_test_data_file(
            'test_suite_psa_crypto_not_supported.generated', test_cases)
