...
SPECIAL_SYSTEMATIC_DEPENDENCIES = {
    ...
    frozenset(['PSA_WANT_ALG_RSA_PKCS1V15_SIGN']),
}
...
def process_data_stanza(stanza, file_name, test_case_number):
    ...
    if len(content_matches) > 3:
        raise Exception('Too many content lines in paragraph {} in {}'.format(
            test_case_number, file_name))
    ...
    new_dependencies = updated_dependencies(file_name, function_name,
                                            arguments, old_dependencies)
    if new_dependencies:
        stanza = (text_before + dependencies_leader +
                  ':'.join(new_dependencies) + text_after)
    else:
