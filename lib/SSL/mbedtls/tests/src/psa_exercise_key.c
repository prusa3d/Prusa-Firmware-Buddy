...
static int check_key_attributes_sanity(mbedtls_svc_key_id_t key) {
    ...
    if (PSA_KEY_LIFETIME_IS_VOLATILE(lifetime)) {
        TEST_ASSERT(
            (PSA_KEY_ID_VOLATILE_MIN <= MBEDTLS_SVC_KEY_ID_GET_KEY_ID(id)) && (MBEDTLS_SVC_KEY_ID_GET_KEY_ID(id) <= PSA_KEY_ID_VOLATILE_MAX));
    } else {
        TEST_ASSERT(
            (PSA_KEY_ID_USER_MIN <= MBEDTLS_SVC_KEY_ID_GET_KEY_ID(id)) && (MBEDTLS_SVC_KEY_ID_GET_KEY_ID(id) <= PSA_KEY_ID_USER_MAX));
    }
    #if defined(MBEDTLS_PSA_CRYPTO_SE_C)
...
static int exercise_asymmetric_encryption_key(mbedtls_svc_key_id_t key,
    ...
    if (usage & PSA_KEY_USAGE_DECRYPT) {
        psa_status_t status = psa_asymmetric_decrypt(key, alg,
            ciphertext, ciphertext_length,
            NULL, 0,
            plaintext, sizeof(plaintext),
            &plaintext_length);
        TEST_ASSERT(status == PSA_SUCCESS || ((usage & PSA_KEY_USAGE_ENCRYPT) == 0 && (status == PSA_ERROR_INVALID_ARGUMENT || status == PSA_ERROR_INVALID_PADDING)));
    }
...
psa_status_t mbedtls_test_psa_raw_key_agreement_with_self(
    ...
    if (status == PSA_SUCCESS) {
        TEST_ASSERT(output_length <= PSA_RAW_KEY_AGREEMENT_OUTPUT_SIZE(private_key_type,
                        key_bits));
...
static int exercise_export_key(mbedtls_svc_key_id_t key,
    ...
    if ((usage & PSA_KEY_USAGE_EXPORT) == 0 && !PSA_KEY_TYPE_IS_PUBLIC_KEY(psa_get_key_type(&attributes))) {
...
psa_key_usage_t mbedtls_test_psa_usage_to_exercise(psa_key_type_t type,
    ...
    if (PSA_ALG_IS_MAC(alg) || PSA_ALG_IS_SIGN(alg)) {
        return (PSA_KEY_TYPE_IS_PUBLIC_KEY(type) ? PSA_KEY_USAGE_VERIFY_HASH : PSA_KEY_USAGE_SIGN_HASH | PSA_KEY_USAGE_VERIFY_HASH);
