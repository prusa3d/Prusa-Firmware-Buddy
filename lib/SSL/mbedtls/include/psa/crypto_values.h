...
#define PSA_ALG_TRUNCATED_MAC(mac_alg, mac_length) \
    (((mac_alg) & ~(PSA_ALG_MAC_TRUNCATION_MASK | PSA_ALG_MAC_AT_LEAST_THIS_LENGTH_FLAG)) | ((mac_length) << PSA_MAC_TRUNCATION_OFFSET & PSA_ALG_MAC_TRUNCATION_MASK))
...
#define PSA_ALG_AEAD_WITH_SHORTENED_TAG(aead_alg, tag_length) \
    (((aead_alg) & ~(PSA_ALG_AEAD_TAG_LENGTH_MASK | PSA_ALG_AEAD_AT_LEAST_THIS_LENGTH_FLAG)) | ((tag_length) << PSA_AEAD_TAG_LENGTH_OFFSET & PSA_ALG_AEAD_TAG_LENGTH_MASK))
...
#define PSA_ALG_IS_WILDCARD(alg) \
    (PSA_ALG_IS_HASH_AND_SIGN(alg) ? PSA_ALG_SIGN_GET_HASH(alg) == PSA_ALG_ANY_HASH : PSA_ALG_IS_MAC(alg) ? (alg & PSA_ALG_MAC_AT_LEAST_THIS_LENGTH_FLAG) != 0 : PSA_ALG_IS_AEAD(alg) ? (alg & PSA_ALG_AEAD_AT_LEAST_THIS_LENGTH_FLAG) != 0 : (alg) == PSA_ALG_ANY_HASH)
...
static inline int mbedtls_svc_key_id_equal(mbedtls_svc_key_id_t id1,
    mbedtls_svc_key_id_t id2) {
    return ((id1.key_id == id2.key_id) && mbedtls_key_owner_id_equal(id1.owner, id2.owner));
}
