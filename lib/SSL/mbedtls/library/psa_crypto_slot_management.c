...
static psa_status_t psa_get_and_lock_key_slot_in_memory(
    ...
    if (psa_key_id_is_volatile(key_id)) {
         ...
         */
        status = mbedtls_svc_key_id_equal(key, slot->attr.id) ? PSA_SUCCESS : PSA_ERROR_DOES_NOT_EXIST;
    } else {
...
psa_status_t psa_get_empty_key_slot(psa_key_id_t *volatile_key_id,
    ...
    if (selected_slot != NULL) {
        ...
        *volatile_key_id = PSA_KEY_ID_VOLATILE_MIN + ((psa_key_id_t)(selected_slot - global_data.key_slots));
