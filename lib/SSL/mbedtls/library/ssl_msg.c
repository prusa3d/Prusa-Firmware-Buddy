...
static int ssl_double_retransmit_timeout(mbedtls_ssl_context *ssl) {
    ...
    MBEDTLS_SSL_DEBUG_MSG(3, ("update timeout value to %lu millisecs", (unsigned long)ssl->handshake->retransmit_timeout));
 ...
 */
    ...
    #if defined(MBEDTLS_SSL_DTLS_CONNECTION_ID) || defined(MBEDTLS_SSL_PROTO_TLS1_3_EXPERIMENTAL)
...
int mbedtls_ssl_encrypt_buf(mbedtls_ssl_context *ssl,
    ...
    #if !(defined(MBEDTLS_SSL_SOME_SUITES_USE_CBC) && (defined(MBEDTLS_SSL_PROTO_TLS1_1) || defined(MBEDTLS_SSL_PROTO_TLS1_2)))
    ((void)f_rng);
    ((void)p_rng);
    #endif
    ...
    if (rec->data_len > MBEDTLS_SSL_OUT_CONTENT_LEN) {
        MBEDTLS_SSL_DEBUG_MSG(1, ("Record content %" MBEDTLS_PRINTF_SIZET " too large, maximum %" MBEDTLS_PRINTF_SIZET, rec->data_len, (size_t)MBEDTLS_SSL_OUT_CONTENT_LEN));
    ...
    if (mode == MBEDTLS_MODE_STREAM) {
        ...
        if (ssl_transform_aead_dynamic_iv_is_explicit(transform) == 1) {
            if (rec->data_len < dynamic_iv_len) {
                MBEDTLS_SSL_DEBUG_MSG(1, ("msglen (%" MBEDTLS_PRINTF_SIZET " ) < explicit_iv_len (%" MBEDTLS_PRINTF_SIZET ") ", rec->data_len, dynamic_iv_len));
        ...
        if (rec->data_len < transform->taglen) {
            MBEDTLS_SSL_DEBUG_MSG(1, ("msglen (%" MBEDTLS_PRINTF_SIZET ") < taglen (%" MBEDTLS_PRINTF_SIZET ") ", rec->data_len, transform->taglen));
            return (MBEDTLS_ERR_SSL_INVALID_MAC);
        }
        ...
        if (rec->data_len < minlen + transform->ivlen || rec->data_len < minlen + transform->maclen + 1) {
            MBEDTLS_SSL_DEBUG_MSG(1, ("msglen (%" MBEDTLS_PRINTF_SIZET ") < max( ivlen(%" MBEDTLS_PRINTF_SIZET "), maclen (%" MBEDTLS_PRINTF_SIZET ") "
                                      "+ 1 ) ( + expl IV )",
        ...
        if (rec->data_len % transform->ivlen != 0) {
            MBEDTLS_SSL_DEBUG_MSG(1, ("msglen (%" MBEDTLS_PRINTF_SIZET ") %% ivlen (%" MBEDTLS_PRINTF_SIZET ") != 0", rec->data_len, transform->ivlen));
            return (MBEDTLS_ERR_SSL_INVALID_MAC);
        }
        ...
        if (auth_done == 1) {
            ...
            if (rec->data_len < transform->maclen + padlen + 1) {
                MBEDTLS_SSL_DEBUG_MSG(1, ("msglen (%" MBEDTLS_PRINTF_SIZET ") < maclen (%" MBEDTLS_PRINTF_SIZET ") + padlen (%" MBEDTLS_PRINTF_SIZET ")", rec->data_len, transform->maclen, padlen + 1));
            }
        #endif
...
static int ssl_compress_buf(mbedtls_ssl_context *ssl) {
    ...
    MBEDTLS_SSL_DEBUG_MSG(3, ("before compression: msglen = %" MBEDTLS_PRINTF_SIZET ", ", ssl->out_msglen));
    ...
    MBEDTLS_SSL_DEBUG_MSG(3, ("after compression: msglen = %" MBEDTLS_PRINTF_SIZET ", ", ssl->out_msglen));
...
static int ssl_decompress_buf(mbedtls_ssl_context *ssl) {
    ...
    MBEDTLS_SSL_DEBUG_MSG(3, ("before decompression: msglen = %" MBEDTLS_PRINTF_SIZET ", ", ssl->in_msglen));
    ...
    MBEDTLS_SSL_DEBUG_MSG(3, ("after decompression: msglen = %" MBEDTLS_PRINTF_SIZET ", ", ssl->in_msglen));
...
int mbedtls_ssl_fetch_input(mbedtls_ssl_context *ssl, size_t nb_want) {
    ...
    if (ssl->conf->transport == MBEDTLS_SSL_TRANSPORT_DATAGRAM) {
        ...
        if (ssl->next_record_offset != 0) {
            ...
            if (ssl->in_left != 0) {
                MBEDTLS_SSL_DEBUG_MSG(2, ("next record in same datagram, offset: %" MBEDTLS_PRINTF_SIZET, ssl->next_record_offset));
                memmove(ssl->in_hdr,
        ...
        MBEDTLS_SSL_DEBUG_MSG(2, ("in_left: %" MBEDTLS_PRINTF_SIZET ", nb_want: %" MBEDTLS_PRINTF_SIZET, ssl->in_left, nb_want));
...
int mbedtls_ssl_flush_output(mbedtls_ssl_context *ssl) {
    ...
    while (ssl->out_left > 0) {
        MBEDTLS_SSL_DEBUG_MSG(2, ("message length: %" MBEDTLS_PRINTF_SIZET ", out_left: %" MBEDTLS_PRINTF_SIZET, mbedtls_ssl_out_hdr_len(ssl) + ssl->out_msglen, ssl->out_left));
...
static int ssl_buffer_message(mbedtls_ssl_context *ssl) {
    ...
    case MBEDTLS_SSL_MSG_HANDSHAKE: {
        ...
        if (!hs_buf->is_valid) {
            ...
            if (reassembly_buf_sz > (MBEDTLS_SSL_DTLS_MAX_BUFFERING - hs->buffering.total_bytes_buffered)) {
                ...
                if (ssl_buffer_make_space(ssl, reassembly_buf_sz) != 0) {
                    MBEDTLS_SSL_DEBUG_MSG(2, ("Reassembly of next message of size %" MBEDTLS_PRINTF_SIZET " (%" MBEDTLS_PRINTF_SIZET " with bitmap) would exceed"
                                              " the compile-time limit %" MBEDTLS_PRINTF_SIZET " (already %" MBEDTLS_PRINTF_SIZET " bytes buffered) -- fail\n",
                                                 msg_len, reassembly_buf_sz, (size_t)MBEDTLS_SSL_DTLS_MAX_BUFFERING, hs->buffering.total_bytes_buffered));
        ...
        if (!hs_buf->is_complete) {
            ...
            MBEDTLS_SSL_DEBUG_MSG(2, ("adding fragment, offset = %" MBEDTLS_PRINTF_SIZET ", length = %" MBEDTLS_PRINTF_SIZET, frag_off, frag_len));
