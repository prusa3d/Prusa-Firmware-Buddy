...
static int ssl_parse_use_srtp_ext(mbedtls_ssl_context *ssl,
    ...
    if ((ssl->conf->transport != MBEDTLS_SSL_TRANSPORT_DATAGRAM) || (ssl->conf->dtls_srtp_profile_list == NULL) || (ssl->conf->dtls_srtp_profile_list_len == 0)) {
        return (0);
    }
    ...
    if (profile_length > len - size_of_lengths || profile_length % 2 != 0) /* profiles are 2 bytes long, so the length must be even */
    {
        mbedtls_ssl_send_alert_message(ssl, MBEDTLS_SSL_ALERT_LEVEL_FATAL,
            MBEDTLS_SSL_ALERT_MSG_HANDSHAKE_FAILURE);
        return (MBEDTLS_ERR_SSL_BAD_HS_CLIENT_HELLO);
    ...
    for (j = 0; j < profile_length; j += 2) {
        ...
        if (client_protection != MBEDTLS_TLS_SRTP_UNSET) {
            MBEDTLS_SSL_DEBUG_MSG(3, ("found srtp profile: %s", mbedtls_ssl_get_srtp_profile_as_string(client_protection)));
        } else {
            continue;
        }
        ...
        for (i = 0; i < ssl->conf->dtls_srtp_profile_list_len; i++) {
            if (client_protection == ssl->conf->dtls_srtp_profile_list[i]) {
                ...
                MBEDTLS_SSL_DEBUG_MSG(3, ("selected srtp profile: %s", mbedtls_ssl_get_srtp_profile_as_string(client_protection)));
                break;
            }
        }
...
static int ssl_parse_client_hello_v2(mbedtls_ssl_context *ssl) {
    ...
    MBEDTLS_SSL_DEBUG_MSG(3, ("ciph_len: %u, sess_len: %u, chal_len: %u", ciph_len, sess_len, chal_len));
...
static void ssl_write_use_srtp_ext(mbedtls_ssl_context *ssl,
    ...
    *olen = 0;
    ...
    if ((ssl->conf->transport != MBEDTLS_SSL_TRANSPORT_DATAGRAM) || (ssl->dtls_srtp_info.chosen_dtls_srtp_profile == MBEDTLS_TLS_SRTP_UNSET)) {
        return;
    }
...
static int ssl_write_server_hello(mbedtls_ssl_context *ssl) {
    ...
    *p++ = (unsigned char)(ssl->session_negotiate->compression);
    ...
    MBEDTLS_SSL_DEBUG_MSG(3, ("server hello, compress alg.: 0x%02X", (unsigned int)ssl->session_negotiate->compression));
