...
static int ssl_write_use_srtp_ext(mbedtls_ssl_context *ssl,
    ...
    *p++ = (unsigned char)((2 * ssl->conf->dtls_srtp_profile_list_len)
    ...
    for (protection_profiles_index = 0;
        ...
        profile_value = mbedtls_ssl_check_srtp_profile_value(ssl->conf->dtls_srtp_profile_list[protection_profiles_index]);
...
static int ssl_write_client_hello(mbedtls_ssl_context *ssl) {
    ...
    *p++ = (unsigned char)n;
    ...
    for (i = 0; ciphersuites[i] != 0; i++) {
        ...
        MBEDTLS_SSL_DEBUG_MSG(3, ("client hello, add ciphersuite: %#04x (%s)", (unsigned int)ciphersuites[i], ciphersuite_info->name));
...
static int ssl_parse_use_srtp_ext(mbedtls_ssl_context *ssl,
    ...
    for (i = 0; i < ssl->conf->dtls_srtp_profile_list_len; i++) {
        if (server_protection == ssl->conf->dtls_srtp_profile_list[i]) {
            ...
            MBEDTLS_SSL_DEBUG_MSG(3, ("selected srtp profile: %s", mbedtls_ssl_get_srtp_profile_as_string(server_protection)));
            break;
        }
    }
...
static int ssl_parse_server_hello(mbedtls_ssl_context *ssl) {
    ...
    MBEDTLS_SSL_DEBUG_MSG(3, ("server hello, current time: %lu", ((unsigned long)buf[2] << 24) | ((unsigned long)buf[3] << 16) | ((unsigned long)buf[4] << 8) | ((unsigned long)buf[5])));
...
static int ssl_parse_server_dh_params(mbedtls_ssl_context *ssl,
    ...
    if (ssl->handshake->dhm_ctx.len * 8 < ssl->conf->dhm_min_bitlen) {
        MBEDTLS_SSL_DEBUG_MSG(1, ("DHM prime too short: %" MBEDTLS_PRINTF_SIZET " < %u", ssl->handshake->dhm_ctx.len * 8, ssl->conf->dhm_min_bitlen));
