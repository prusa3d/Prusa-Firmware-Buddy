...
class ConfigFile(Config):
    ...
    _define_line_regexp = (r'(?P<indentation>\s*)' +
                           r'(?P<commented_out>(//\s*)?)' +
                           r'(?P<define>#\s*define\s+)' + r'(?P<name>\w+)' +
                           r'(?P<arguments>(?:\((?:\w|\s|,)*\))?)' +
                           r'(?P<separator>\s*)' + r'(?P<value>.*)')
    _section_line_regexp = (r'\s*/?\*+\s*[\\@]name\s+SECTION:\s*' +
                            r'(?P<section>.*)[ */]*')
    _config_line_regexp = re.compile(r'|'.join(
        [_define_line_regexp, _section_line_regexp]))
    ...
    def _parse_line(self, line):
        ...
        if m is None:
            ...
            name = m.group('name')
            value = m.group('value')
            template = (name, m.group('indentation'), m.group('define') +
                        name + m.group('arguments') + m.group('separator'))
    ...
    def _format_template(self, name, indent, middle):
        ...
        return ''.join([indent, '' if setting.active else '//', middle,
                        value]).rstrip()
...
if __name__ == '__main__':
    ...
    def main():
        ...
        add_adapter(
            'full', full_adapter, """Uncomment most features.
        ...
        add_adapter(
            ...
            """Uncomment most non-deprecated features.
                    ...
                    """)
        add_adapter(
            'realfull', realfull_adapter, """Uncomment all boolean #defines.
