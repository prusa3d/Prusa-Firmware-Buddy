...
class TextChangelogFormat(ChangelogFormat):
    ...
    def extract_top_version(cls, changelog_file_content):
        ...
        if cls.is_released_version(top_version_title):
            ...
            top_version_body = b''
        return (changelog_file_content[:top_version_start], top_version_title,
                top_version_body, changelog_file_content[top_version_end:])
...
class ChangeLog:
    ...
    def add_categories_from_text(self, filename, line_offset, text,
    ...
    def __init__(self, input_stream, changelog_format):
        ...
        (self.header, self.top_version_title, top_version_body,
...
class EntryFileSortKey:
    ...
    * Uncommitted entry files are sorted according to their modification time.
    ...
    def merge_hash(cls, some_hash):
        ...
        ancestry = frozenset(
            cls.list_merges(some_hash, target, '--ancestry-path'))
        ...
        first_parents = cls.list_merges(some_hash, target, '--first-parent',
                                        '--reverse')
