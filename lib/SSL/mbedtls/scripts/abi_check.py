...
class AbiChecker:
    ...
    def __init__(self, old_version, new_version, configuration):
        ...
        self.can_remove_report_dir = not (os.path.exists(self.report_dir)
                                          or self.keep_all_reports)
    ...
    def _get_clean_worktree_for_git_revision(self, version):
        ...
        worktree_output = subprocess.check_output([
            self.git_command, "worktree", "add", "--detach", git_worktree_path,
            worktree_rev
        ],
                                                  cwd=self.repo_path,
                                                  stderr=subprocess.STDOUT)
    ...
    def _build_shared_libraries(self, git_worktree_path, version):
        ...
        make_output = subprocess.check_output([self.make_command, "lib"],
                                              env=my_environment,
                                              cwd=git_worktree_path,
                                              stderr=subprocess.STDOUT)
        ...
        for root, _dirs, files in os.walk(git_worktree_path):
            for file in fnmatch.filter(files, "*.so"):
                version.modules[os.path.splitext(file)[0]] = (os.path.join(
                    root, file))
    ...
    def _get_abi_dumps_from_shared_libraries(self, version):
        ...
        for mbed_module, module_path in version.modules.items():
            output_path = os.path.join(
                self.report_dir,
                "{}-{}-{}.dump".format(mbed_module, version.revision,
                                       version.version))
            abi_dump_command = [
                "abi-dumper",
                module_path,
            ...
            abi_dump_output = subprocess.check_output(abi_dump_command,
                                                      stderr=subprocess.STDOUT)
    ...
    def _abi_compliance_command(self, mbed_module, output_path):
        ...
        if self.skip_file:
            abi_compliance_command += [
                "-skip-symbols", self.skip_file, "-skip-types", self.skip_file
            ]
        if self.brief:
    ...
    def _is_library_compatible(self, mbed_module, compatibility_report):
        ...
        output_path = os.path.join(
            self.report_dir,
            "{}-{}-{}.html".format(mbed_module, self.old_version.revision,
                                   self.new_version.revision))
        try:
            subprocess.check_output(self._abi_compliance_command(
                mbed_module, output_path),
                                    stderr=subprocess.STDOUT)
