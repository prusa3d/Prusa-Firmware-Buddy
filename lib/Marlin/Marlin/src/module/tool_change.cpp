    ...
    if (new_tool != old_tool) {
      ...
      REMEMBER(fr, feedrate_mm_s, XY_PROBE_FEEDRATE_MM_S);
        ...
        if (prusa_toolchanger.tool_change(new_tool)) {
          if (can_move_away && return_type == tool_return_t::no_return) {
            // just undo the Z lift immediately
            current_position.z -= toolchange_settings.z_raise;
            planner.buffer_line(current_position, planner.settings.max_feedrate_mm_s[Z_AXIS], new_tool);
          } else {
            // update return_position to the new working offset
            return_position += prusa_toolchanger.last_tool_change_offset();
          }
        } else {
          // toolchange failed: do not return
          return_type = tool_return_t::no_move;
        }
      #endif
      ...
      if (safe_to_move && IsRunning()) {
        ...
        if ((return_type > tool_return_t::no_return) && all_axes_known()) {
            ...
            #if ENABLED(PRUSA_TOOLCHANGER)
              // schedule a smooth XY transition to return_position
              prusa_toolchanger.unpark_to(return_position);
            #else
