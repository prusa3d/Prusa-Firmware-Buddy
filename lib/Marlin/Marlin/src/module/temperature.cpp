...
#define HEATER_PSTR(h) _BED_PSTR(h) \
_CHAMBER_PSTR(h) _E_PSTR(h, 1) _E_PSTR(h, 2) _E_PSTR(h, 3) _E_PSTR(h, 4) _E_PSTR(h, 5) PSTR(LCD_STR_E0)
...
void Temperature::set_fan_speed(uint8_t target, uint16_t speed) {
    ...
    if (target != active_extruder) {
        if (target < EXTRUDERS)
            singlenozzle_fan_speed[target] = speed;
        return;
    }
...
void Temperature::thermal_runaway_protection(Temperature::tr_state_machine_t &sm, const float &current, const float &target, const heater_ind_t heater_id, const uint16_t period_seconds, const uint16_t hysteresis_degc_up, const uint16_t hysteresis_degc_down) {
    ...
    case TRRunaway:
        if (heater_id == H_BED)
            _temp_error(heater_id, PSTR(MSG_T_THERMAL_RUNAWAY), GET_TEXT(MSG_THERMAL_RUNAWAY_BED));
        else
    ...
    do {
        ...
        if (!residency_start_ms) {
            ...
            if (temp_diff < TEMP_WINDOW) {
                ...
                if (first_loop)
                    residency_start_ms -= (TEMP_RESIDENCY_TIME)*1000UL;
            }
