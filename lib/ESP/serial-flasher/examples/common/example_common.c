...
esp_loader_error_t connect_to_target(uint32_t higher_baudrate)
{
    ...
    if (higher_baudrate && esp_loader_get_target() != ESP8266_CHIP) {
        ...
        if (err == ESP_LOADER_ERROR_UNSUPPORTED_FUNC) {
            _dbg0("ESP8266 does not support change baudrate command.");
            return err;
        } else if (err != ESP_LOADER_SUCCESS) {
            _dbg0("Unable to change baud rate on target.");
            return err;
        } else {
            err = loader_port_change_baudrate(higher_baudrate);
            if (err != ESP_LOADER_SUCCESS) {
                _dbg0("Unable to change baud rate.");
                return err;
            }
            _dbg0("Baudrate changed\n");
        }
    }
