...
GCodePreview::GCodePreview(FILE *f, bool can_keep_alive, uint16_t width, uint16_t height)
    : gcode(f)
...
Step GCodePreview::step(string_view, bool, uint8_t *buffer, size_t buffer_size) {
    ...
    if (!headers_sent) {
        ...
        if (got > 0) {
            ...
            size_t written = write_headers(buffer, buffer_size - reserve, Status::Ok, ContentType::ImagePng, handling);
