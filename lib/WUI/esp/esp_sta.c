 ...
 */
esp_res_t
esp_sta_reconnect_set_config(uint16_t interval, uint16_t rep_cnt,
                            const esp_api_cmd_evt_fn evt_fn, void* const evt_arg, const uint32_t blocking) {
    ESP_MSG_VAR_DEFINE(msg);

    ESP_ASSERT("interval <= 7200", interval <= 7200);
    ESP_ASSERT("rep_cnt <= 1000", rep_cnt <= 1000);

    ESP_MSG_VAR_ALLOC(msg, blocking);
    ESP_MSG_VAR_SET_EVT(msg, evt_fn, evt_arg);
    ESP_MSG_VAR_REF(msg).cmd_def = ESP_CMD_WIFI_CWRECONNCFG;
    ESP_MSG_VAR_REF(msg).msg.sta_reconn_set.interval = interval;
    ESP_MSG_VAR_REF(msg).msg.sta_reconn_set.rep_cnt = rep_cnt;
