 ...
 */
#define ESP_SET_IP(ip_str, ip1, ip2, ip3, ip4) \
    do {                                       \
        (ip_str)->ip[0] = (ip1);               \
        (ip_str)->ip[1] = (ip2);               \
        (ip_str)->ip[2] = (ip3);               \
 ...
 * \return          \ref espOK on success, member of \ref esp_res_t otherwise
 */
typedef esp_res_t (*esp_evt_fn)(struct esp_evt *evt);
...
typedef struct esp_evt {
    ...
    union {
        ...
        struct {
            esp_res_t res; /*!< Reset operation result */
        } reset;           /*!< Reset sequence finish. Use with \ref ESP_EVT_RESET event */
        struct {
            esp_res_t res; /*!< Restore operation result */
        ...
        struct {
            ...
            esp_res_t res;   /*!< Send data result */
        ...
        struct {
            ...
            esp_res_t res;   /*!< Result of close event.
                                                        Set to \ref espOK on success */
        } conn_active_close; /*!< Process active and closed statuses at the same time.
                                                        Use with \ref ESP_EVT_CONN_ACTIVE or
                                                        \ref ESP_EVT_CONN_CLOSE events */
        struct {
            esp_conn_p conn; /*!< Set connection pointer */
        } conn_poll;         /*!< Polling active connection to check for timeouts.
                                                        Use with \ref ESP_EVT_CONN_POLL event */

        struct {
            esp_res_t res;   /*!< Status of command */
...
#if ESP_CFG_MODE_STATION || __DOXYGEN__
        struct {
            esp_res_t res; /*!< Result of command */
        ...
        struct {
            esp_mac_t *mac;    /*!< Station MAC address */
                                                        ...
                                                        \ref ESP_EVT_AP_CONNECTED_STA or
                                                        \ref ESP_EVT_AP_DISCONNECTED_STA events */
        struct {
            esp_mac_t *mac; /*!< Station MAC address */
            esp_ip_t *ip;   /*!< Station IP address */
        } ap_ip_sta;        /*!< Station got IP address from ESP's access point.
                                                        Use with \ref ESP_EVT_AP_IP_STA event */
#endif                      /* ESP_CFG_MODE_ACCESS_POINT || __DOXYGEN__ */
#if ESP_CFG_DNS || __DOXYGEN__
        struct {
            esp_res_t res;    /*!< Result of command */
            ...
            esp_ip_t *ip;     /*!< Pointer to IP result */
                                                        ...
                                                        Use with \ref ESP_EVT_DNS_HOSTBYNAME event */
#endif                        /* ESP_CFG_DNS || __DOXYGEN__ */
#if ESP_CFG_PING || __DOXYGEN__
        struct {
            esp_res_t res;    /*!< Result of command */
 ...
 * \param[in]       res: Operation result, member of \ref esp_res_t enumeration
 * \param[in]       arg: Custom user argument
 */
typedef void (*esp_api_cmd_evt_fn)(esp_res_t res, void *arg);
