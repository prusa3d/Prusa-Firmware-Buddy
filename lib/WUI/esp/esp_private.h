...
typedef enum {
    ...
    ESP_CMD_WIFI_CWRECONNCFG,   /*!< Setup reconnect interval and maximum tries */
...
esp_res_t espi_process(const void *data, size_t len);
esp_res_t espi_process_buffer(void);
esp_res_t espi_initiate_cmd(esp_msg_t *msg);
uint8_t espi_is_valid_conn_ptr(esp_conn_p conn);
esp_res_t espi_send_cb(esp_evt_type_t type);
esp_res_t espi_send_conn_cb(esp_conn_t *conn, esp_evt_fn cb);
