...
static esp_res_t espi_process_sub_cmd(esp_msg_t *msg, uint8_t *is_ok, uint8_t *is_error, uint8_t *is_ready);
 ...
 * \param[in]       err: Error of type \ref esp_res_t
 */
 ...
 */
void espi_send_ip_mac(const void *d, uint8_t is_ip, uint8_t q, uint8_t c) {
    uint8_t ch;
    char str[4];
 ...
 * \param[in]       c: Set to `1` to include comma before string
 */
void espi_send_string(const char *str, uint8_t e, uint8_t q, uint8_t c) {
    char special = '\\';
 ...
 * \param[in]       c: Set to `1` to include comma before string
 */
void espi_send_number(uint32_t num, uint8_t q, uint8_t c) {
    char str[11];
 ...
 * \param[in]       c: Set to `1` to include comma before string
 */
void espi_send_port(esp_port_t port, uint8_t q, uint8_t c) {
    char str[6];
 ...
 * \param[in]       c: Set to `1` to include comma before string
 */
void espi_send_signed_number(int32_t num, uint8_t q, uint8_t c) {
    char str[11];
 ...
 * \return          Member of \ref esp_res_t enumeration
 */
esp_res_t
 ...
 */
static void
espi_send_conn_error_cb(esp_msg_t *msg, esp_res_t error) {
...
#endif /* ESP_CFG_MODE_STATION */
#if ESP_CFG_MODE_ACCESS_POINT
    } else if (CMD_IS_DEF(ESP_CMD_WIFI_CWMODE) && (msg->msg.wifi_mode.mode == ESP_MODE_AP
    #if ESP_CFG_MODE_STATION
...
/*         if (esp.ll.reset_fn != NULL && esp.ll.reset_fn(1)) { */
    ...
    case ESP_CMD_WIFI_CWRECONNCFG: { /* Set reconnect interval */
        AT_PORT_SEND_BEGIN_AT();
        AT_PORT_SEND_CONST_STR("+CWRECONNCFG=");
        espi_send_number(msg->msg.sta_reconn_set.interval, 0, 0);
        //if (msg->msg.sta_reconn_set.interval > 0) {
        espi_send_number(msg->msg.sta_reconn_set.rep_cnt, 0, 1);
        //}
        AT_PORT_SEND_END_AT();
        break;
    }
 ...
 */
esp_res_t
espi_send_msg_to_producer_mbox(esp_msg_t *msg, esp_res_t (*process_fn)(esp_msg_t *), uint32_t max_block_time) {
