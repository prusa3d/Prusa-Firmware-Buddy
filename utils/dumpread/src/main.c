...
int main(int argc, char** argv)
{
    ...
    strcpy(dump_fn, "d:\\Projects\\Prusa3D\\STM32\\PFW-Buddy-HARDFAULT\\DUMP_FW194\\"_PRINTER"\\Dump\\dump.bin");
    ...
    strcpy(out_dir, "d:\\Projects\\Prusa3D\\STM32\\PFW-Buddy-HARDFAULT\\DUMP_FW194\\"_PRINTER"\\Dump\\");
    ...
    {
        arg = argv[argn];
        if (sscanf(arg, "-dump=%s", dump_fn) == 1) continue;
        if (sscanf(arg, "-map=%s", map_fn) == 1) continue;
        if (sscanf(arg, "-bin=%s", bin_fn) == 1) continue;
        if (sscanf(arg, "-out=%s", out_dir) == 1) continue;
    }
    //check args
