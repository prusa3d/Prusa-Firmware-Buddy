...
public:
    WI_TEMP_LABEL_t(const string_view_utf8 &label, const img::Resource *id_icon, is_enabled_t enabled, is_hidden_t hidden)
        : WI_FORMATABLE_LABEL_t<float>(label, id_icon, enabled, hidden, {}, [&](char *buffer) {
            snprintf(buffer, GuiDefaults::infoDefaultLen, "%.1f\xC2\xB0\x43", (double)value); // \xB0\x43 == degree Celsius
        }) {}
};
