...
public:
    WI_LAMBDA_LABEL_t(string_view_utf8 label, png::Id id_icon, is_enabled_t enabled, is_hidden_t hidden, std::function<void(char *)> printAs)
        : AddSuper<WI_LABEL_t>(label, !id_icon.IsNull() ? icon_width : GuiDefaults::infoDefaultLen * InfoFont->w, id_icon, enabled, hidden)
        , printAs(printAs) {}
};
...
public:
    WI_FORMATABLE_LABEL_t(string_view_utf8 label, png::Id id_icon, is_enabled_t enabled, is_hidden_t hidden, ValueType initVal, std::function<void(char *)> printAs)
