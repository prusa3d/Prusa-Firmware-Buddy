...
Download::BeginResult Download::begin(const Request &request, DestinationPath destination, uint32_t offset) {
    // prepare headers
    ...
    {
        bool include_range_header = offset != 0;
    ...
    if (auto resp_any = send_request(request.host, request.port, request.url_path, extra_hdrs.get()); resp_any.has_value()) {
        ...
        if (offset != 0 && resp.status != Status::PartialContent) {
        ...
        uint32_t file_size = request.encryption != nullptr ? request.encryption->orig_size : resp.content_length() + offset;
        ...
        if (request.encryption) {
            ...
            case ContentEncryptionMode::AES_CBC:
                decryptor = make_unique<Decryptor>(request.encryption->key, Decryptor::CBC(request.encryption->iv), file_size - offset);
                break;
            case ContentEncryptionMode::AES_CTR:
                decryptor = make_unique<Decryptor>(request.encryption->key, Decryptor::CTR(request.encryption->nonce, offset), file_size - offset);
                break;
            default:
