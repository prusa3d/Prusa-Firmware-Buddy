...
static bool ensure_safe_temperature_notify_progress(PhasesLoadUnload phase, uint8_t progress_min, uint8_t progress_max) {
    ...
    Notifier_TEMP_NOZ N(FSM_load_unload, GetPhaseIndex(phase),
...
void do_pause_e_move_notify_progress(const float &length, const feedRate_t &fr_mm_s, PhasesLoadUnload phase, uint8_t progress_min, uint8_t progress_max) {
    ...
    //Notifier_POS_E N(FSM_load_unload, GetPhaseIndex(phase), actual_e, actual_e + length, progress_min,progress_max);
    const float actual_e = current_position.e;
    current_position.e += length / planner.e_factor[active_extruder];
    Notifier_POS_E N(FSM_load_unload, GetPhaseIndex(phase), actual_e, current_position.e, progress_min, progress_max);
    line_to_current_position(fr_mm_s);
    planner.synchronize();
}
...
bool load_filament(const float &slow_load_length /*=0*/, const float &fast_load_length /*=0*/, const float &purge_length /*=0*/, const int8_t max_beep_count /*=0*/,
    ...
    change_dialog_handler(FSM_load_unload, GetPhaseIndex(PhasesLoadUnload::UserPush), 30, 0);
    ...
    if (purge_length > 0) {
        ...
        do {
            ...
            change_dialog_handler(FSM_load_unload, GetPhaseIndex(PhasesLoadUnload::IsColor), 99, 0);
            do {
                idle();
...
bool unload_filament(const float &unload_length, const bool show_lcd /*=false*/,
    ...
    change_dialog_handler(FSM_load_unload, GetPhaseIndex(PhasesLoadUnload::Ramming), 50, 0);
...
void wait_for_confirmation(const bool is_reload /*=false*/, const int8_t max_beep_count /*=0*/ DXC_ARGS) {
    ...
    change_dialog_handler(FSM_load_unload, GetPhaseIndex(PhasesLoadUnload::UserPush), -1, 0);
    ...
    while (wait_for_user && (ServerDialogCommands::GetCommandFromPhase(PhasesLoadUnload::UserPush) != Command::Continue)) {
        ...
        if (nozzle_timed_out) {
            ...
            change_dialog_handler(FSM_load_unload, GetPhaseIndex(PhasesLoadUnload::NozzleTimeout), -1, 0);
