...
xy_pos_t probe_xy(const xyz_pos_t center, const float angle) {
...
xy_pos_t probe_xy_verify(const xyz_pos_t center, const float angle, const float probe_distance) {
    ...
    for (uint i = 0; i < NUM_PROBE_TRIES; ++i) {
        ...
        hits[i % hits.size()] = probe_xy(center, angle);
    }
...
float probe_z(const xyz_pos_t position, float uncertainty, const int num_measurements) {
    ...
    for (int i = 0; i < num_measurements; i++) {
        ...
        if (std::isnan(measurement)) {
            kill("PROBING ERROR", "Probe not successful");
        }
...
const xyz_pos_t get_xyz_center(const uint phase) {
    ...
    for (uint hit_no = 0; xy_pos_t & hit : initial_hits) {
        hit = probe_xy_verify(start, 2 * PI / initial_hits.size() * hit_no++, PROBE_XY_UNCERTAIN_DIST_MM);
    }
    ...
    for (uint hit_no = 0; xy_pos_t & hit : second_hits) {
        hit = probe_xy_verify(initial_center, 2 * PI / second_hits.size() * hit_no++, PROBE_XY_CERTAIN_DIST_MM);
    }
    ...
    for (uint hit_no = 0; xy_pos_t & hit : hits) {
        hit = probe_xy_verify(second_center, 2 * PI / hits.size() * hit_no++, PROBE_XY_CERTAIN_DIST_MM);
    }
