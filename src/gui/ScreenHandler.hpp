...
struct screen_node {
    screen_t::Creator creator;
    screen_init_variant init_data;
    screen_node(screen_t::Creator creator = nullptr, screen_init_variant init_data = screen_init_variant())
        : creator(creator)
        , init_data(init_data) {}
    void MakeEmpty() {
        creator = nullptr;
...
class Screens {
    ...
    screen_t::UniquePtr current; // pointer obtained by screen creation
...
public:
    const ScreenArray &GetStack() { return stack; }
    const ScreenArray::iterator &GetStackIterator() { return stack_iterator; }

    void Loop(); //call inside guiloop

    void Open(const screen_t::Creator screen_creator); //remember creator and create later with default initialization parameter (like selected item in menu)
    ...
    void PushBeforeCurrent(const screen_t::Creator screen_creator);
