...
public:
    MI_CONNECT_ENABLED()
        : WI_ICON_SWITCH_OFF_ON_t(config_store().connect_enabled.get(), string_view_utf8::MakeCPUFLASH((const uint8_t *)label), IDR_NULL, is_enabled_t::yes, is_hidden_t::no) {}

protected:
    virtual void OnChange(size_t old_index) override {
        config_store().connect_enabled.set(index);
