...
protected:
    ...
    static void writeColorToLeds(const leds::Color &toSet, const std::pair<uint16_t, uint16_t> &leds_to_run);
};
...
public:
    ...
    Fading(leds::Color color_, uint16_t period_);
    void Step(const std::pair<uint16_t, uint16_t> &leds_to_run) override;
    void EndAnimation() override;

private:
    leds::Color calculateColor(uint32_t ticks);
    leds::Color color;
