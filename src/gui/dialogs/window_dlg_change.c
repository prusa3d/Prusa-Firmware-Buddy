...
static int f_CH_WAIT_E_POS__UNLOADING(_dlg_vars *p_vars, _dlg_ld_vars *additional_vars) {
    // 8 ... -392
    float pos = additional_vars->p_marlin_vars->pos[MARLIN_VAR_INDEX_E] - additional_vars->e_start;
    if (pos < (filament_unload_mini_lengt - 1.0F))
        p_vars->phase++;
    return 100 * (8.0F - pos) / (filament_unload_mini_lengt + 8);
}
...
static int f_CH_WAIT_E_POS__INSERTING(_dlg_vars *p_vars, _dlg_ld_vars *additional_vars) {
    ...
    float pos = additional_vars->p_marlin_vars->pos[MARLIN_VAR_INDEX_E] - additional_vars->e_start;
    //wait E pos >= 40
    if (pos >= filament_change_slow_load_lenght)
        p_vars->phase++;
    return 100 * (pos) / filament_change_slow_load_lenght;
}

static int f_CH_WAIT_E_POS__LOADING_TO_NOZ(_dlg_vars *p_vars, _dlg_ld_vars *additional_vars) {
    //wait E pos >= 360
    float pos = additional_vars->p_marlin_vars->pos[MARLIN_VAR_INDEX_E] - additional_vars->e_start;
    if (pos >= filament_change_full_load_lenght)
        p_vars->phase++;
    float ret = 100 * (pos - filament_change_slow_load_lenght) / (filament_change_full_load_lenght - filament_change_slow_load_lenght);
    return ret;
}
...
static int f_CH_WAIT_E_POS__PURGING(_dlg_vars *p_vars, _dlg_ld_vars *additional_vars) {
    //wait E pos >= 400
    float pos = additional_vars->p_marlin_vars->pos[MARLIN_VAR_INDEX_E] - additional_vars->e_start;
    if ((pos >= filament_change_full_purge_load_lenght) && (marlin_motion() == 0))
        p_vars->phase++;
    return 100 * (pos - filament_change_full_load_lenght) / (filament_change_full_purge_load_lenght - filament_change_full_load_lenght);
}
