...
public:
    ...
    ScreenMenuTune()
        ...
        //todo test if needed
        //print_client::update_vars(MARLIN_VAR_MSK_TEMP_TARG | MARLIN_VAR_MSK(MARLIN_VAR_Z_OFFSET) | MARLIN_VAR_MSK(MARLIN_VAR_FANSPEED) | MARLIN_VAR_MSK(MARLIN_VAR_PRNSPEED) | MARLIN_VAR_MSK(MARLIN_VAR_FLOWFACT));
    }
...
void ScreenMenuTune::windowEvent(EventLock /*has private ctor*/, window_t *sender, GUI_event_t event, void *param) {
    ...
    case GUI_event_t::LOOP:
        if (marlin_all_axes_homed() && marlin_all_axes_known() && (print_client::command() != MARLIN_CMD_G28) && (print_client::command() != MARLIN_CMD_G29) && (print_client::command() != MARLIN_CMD_M109) && (print_client::command() != MARLIN_CMD_M190)) {
            Item<MI_M600>().Enable();
        } else {
