    ...
    if (event == GUI_event_t::CHILD_CLICK) {
        ...
        case esp_upload_action::Connect: {
            ...
            if (ESP_LOADER_SUCCESS == esp_loader_connect(&config)) {
                ...
                help.SetText(_("Successfully connected to ESP. Do not switch the Printer off until the update ends."));
        ...
        case esp_upload_action::Start_flash:
            if ((opened_file = fopen(current_file->filename, "rb")) == nullptr) {
                ...
                help.SetText(_("Unable to open files. Try to re-flash the last FW and try again"));
                progress_state = esp_upload_action::ESP_error;
                break;
            } else {
        ...
        case esp_upload_action::Write_data: {
            ...
            if (ferror(opened_file)) {
                ...
                help.SetText(_("Unable to read files. Try re-flash the last FW and try again"));
                readBytes = 0;
                progress_state = esp_upload_action::ESP_error;
                break;
            }
