...
//text
  ...
  /*  //dots
    point_ui16_t pt_txt_ln2 = font_meas_text(resource_font(IDR_FNT_NORMAL), txt+16);
    point_ui16_t pt_dot = font_meas_text(resource_font(IDR_FNT_NORMAL), ".");
    for (size_t i = 0; i < sizeof(pw->txt_dots)/sizeof(pw->txt_dots[0]);++i) {
        id = window_create_ptr(WINDOW_CLS_TEXT, root,
            rect_ui16((240-pt_txt.x)/2+pt_txt_ln2.x + i * pt_dot.x , pt_txt_ln2.y +gui_defaults.msg_box_sz.y + pt_ico.y, 240, 95),
            &(pw->txt_dots[i]));
        window_set_alignment(id, ALIGN_LEFT_TOP);
        window_set_text(id, ".");
    }*/
