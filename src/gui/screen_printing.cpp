...
static void update_end_timestamp(screen_t *screen, time_t now_sec) {
    ...
    if (now.tm_mday == print_end.tm_mday && // if print end is today
        ...
        strftime(pw->text_etime.data(), MAX_END_TIMESTAMP_SIZE, "Tommorow at %H:%M?", &print_end); //@@TODO translate somehow
    } else {
        strftime(pw->text_etime.data(), MAX_END_TIMESTAMP_SIZE, "%m-%d at %H:%M?", &print_end); //@@TODO translate somehow
    }
