...
struct CurrentStore : public journal::CurrentStoreConfig<journal::Backend, backend> {
    ...
    StoreItem<bool, defaults::bool_false, journal::hash("NTP via DHCP")> ntp_via_dhcp; // use dhcp server for ntp
    StoreItem<std::array<char, DNS_MAX_NAME_LENGTH - 68 + 1>, defaults::ntp_server, journal::hash("NTP Address")> ntp_addr; // X.X.X.X address encoded or string
