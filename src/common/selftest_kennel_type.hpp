...
struct SelftestKennel_t {
    uint8_t progress;
    SelftestSubtestState_t state;

    constexpr SelftestKennel_t(uint8_t prog = 0, SelftestSubtestState_t st = SelftestSubtestState_t::undef)
        : progress(prog)
        , state(st) {}

    constexpr bool operator==(const SelftestKennel_t &other) const {
...
class SelftestKennels_t {
    // TODO: This does not handle per kennel progress as it would not fit info FSM data :-(

    struct __attribute__((packed)) SerializedData {
        uint8_t current_kennel;
        uint8_t total_progress;
        uint16_t states;
    };
    static_assert(sizeof(SerializedData) <= sizeof(fsm::PhaseData), "Selftest kennels data fits into Phase data");
...
public:
    ...
    constexpr SelftestKennels_t(uint8_t current_kennel, std::array<SelftestKennel_t, EXTRUDERS> kennels)
        : current_kennel(current_kennel) {
        uint32_t progress_sum = 0;
        for (uint8_t i = 0; i < EXTRUDERS; ++i) {
            progress_sum += kennels[i].progress;
            states[i] = kennels[i].state;
        }
        total_progress = progress_sum / EXTRUDERS;
    }

    constexpr SelftestKennels_t(fsm::PhaseData new_data) {
    ...
    constexpr fsm::PhaseData Serialize() const {
        ...
        serialized.current_kennel = current_kennel;
    ...
    constexpr void Deserialize(fsm::PhaseData new_data) {
        ...
        current_kennel = serialized.current_kennel;
    ...
    constexpr bool operator==(const SelftestKennels_t &other) const {
