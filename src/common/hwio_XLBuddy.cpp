...
const buddy::hw::OutputPin *Buzzer = nullptr;
const buddy::hw::OutputPin *SideLed_LcdSelector = nullptr;
const buddy::hw::OutputPin *XStep = nullptr;
const buddy::hw::OutputPin *YStep = nullptr;
/**
...
int digitalRead(uint32_t marlinPin) {
    ...
    case MARLIN_PIN(X_STEP):
        return XStep->read() == Pin::State::low ? 0 : 1;
    case MARLIN_PIN(Y_STEP):
        return YStep->read() == Pin::State::low ? 0 : 1;
    default:
#if _DEBUG
...
void digitalWrite(uint32_t marlinPin, uint32_t ulVal) {
    ...
    case MARLIN_PIN(X_STEP):
        XStep->write(ulVal ? Pin::State::high : Pin::State::low);
        break;
    case MARLIN_PIN(Y_STEP):
        YStep->write(ulVal ? Pin::State::high : Pin::State::low);
        break;
