...
struct marlin_server_t {
    ...
    PrintState print_state;                        // printing state (printing, paused, ...)
...
public:
    ...
    void checkTrue(bool condition) {
        if (!condition && !m_failed) {
            ...
            if (marlin_server.print_state == PrintState::Printing) {
                marlin_server.print_state = PrintState::Pausing_Begin;
            }
            if (p_disableHotend) {
...
static void marlin_server_check_crash() {
    ...
    if (power_panic::ac_fault_state == power_panic::AcFaultState::Triggered) {
        marlin_server.print_state = PrintState::PowerPanic_acFault;
        return;
    }
    #endif
    if (crash_s.get_state() == Crash_s::TRIGGERED_ISR) {
        marlin_server.print_state = PrintState::CrashRecovery_Begin;
        return;
    }
}
#endif // ENABLED(CRASH_RECOVERY)
...
#if HAS_SELFTEST
    if (((marlin_server.print_state == PrintState::Idle) || (marlin_server.print_state == PrintState::Finished) || (marlin_server.print_state == PrintState::Aborted)) && (!SelftestInstance().IsInProgress())) {
...
bool marlin_server_printer_idle() {
    return marlin_server.print_state == PrintState::Idle
        || marlin_server.print_state == PrintState::Paused
        || marlin_server.print_state == PrintState::Aborted
        || marlin_server.print_state == PrintState::Finished;
}

bool marlin_server_printer_paused() {
    return marlin_server.print_state == PrintState::Paused;
}
...
void marlin_server_print_start(const char *filename, bool skip_preview) {
    ...
    case PrintState::PrintPreviewLoop:
        ...
        marlin_server.print_state = PrintState::WaitGui;
...
void marlin_server_gui_ready_to_print() {
    ...
    case PrintState::WaitGui:
        marlin_server.print_state = PrintState::PrintPreviewInit;
        break;
    default:
        log_error(MarlinServer, "Wrong print state, expected: %d, is: %d", PrintState::WaitGui, marlin_server.print_state);
        break;
    }
}
...
void marlin_server_print_abort(void) {
    ...
    case PrintState::Finishing_WaitIdle:
        marlin_server.print_state = PrintState::Aborting_Begin;
        break;
    default:
        break;
    }
...
void marlin_server_print_exit(void) {
    ...
    case PrintState::Finishing_WaitIdle:
        ...
        break;
    default:
        marlin_server.print_state = PrintState::Exit;
        break;
    }
}
...
void marlin_server_print_resume(void) {
    if (marlin_server.print_state == PrintState::Paused) {
        marlin_server.print_state = PrintState::Resuming_Begin;
#if ENABLED(POWER_PANIC)
    } else if (marlin_server.print_state == PrintState::PowerPanic_AwaitingResume) {
        marlin_server.print_state = PrintState::PowerPanic_Resume;
#endif
    } else
...
void marlin_server_print_reheat_start(void) {
    if ((marlin_server.print_state == PrintState::Paused) && marlin_server_print_reheat_ready()) {
...
void marlin_server_powerpanic_resume_loop(const char *media_SFN_path, uint32_t pos, bool start_paused) {
    ...
    marlin_server.print_state = start_paused ? PrintState::PowerPanic_AwaitingResume : PrintState::PowerPanic_Resume;
...
void marlin_server_powerpanic_finish(bool paused) {
    ...
    if (paused) {
        ...
        marlin_server.print_state = PrintState::Resuming_UnparkHead_ZE;
    }
}
#endif
...
static void marlin_server_resuming_reheating() {
    ...
    marlin_server.print_state = PrintState::Resuming_UnparkHead_XY;
}
...
static void _server_print_loop(void) {
    ...
    case PrintState::Idle:
        break;
    case PrintState::WaitGui:
        // without gui just act as if state == PrintState::PrintPreviewInit
#if HAS_GUI
        break;
#endif
    case PrintState::PrintPreviewInit:
        ...
        marlin_server.print_state = PrintState::PrintPreviewLoop;
        break;
        /*
        ...
        */
    case PrintState::PrintPreviewLoop: // button evaluation
        switch (PrintPreview::Instance().Loop()) {
        case PrintPreview::Result::InProgress:
            break;
        case PrintPreview::Result::Abort:
            marlin_server.print_state = did_not_start_print ? PrintState::Idle : PrintState::Finishing_WaitIdle;
            break;
        case PrintPreview::Result::Print:
        case PrintPreview::Result::Inactive:
            did_not_start_print = false;
            marlin_server.print_state = PrintState::PrintInit;
            break;
        }
        break;
    case PrintState::PrintInit:
        ...
        marlin_server.print_state = PrintState::Printing;
    ...
    case PrintState::Printing:
        ...
        case media_print_state_NONE:
            marlin_server.print_state = PrintState::Finishing_WaitIdle;
            break;
    ...
    case PrintState::Pausing_Failed_Code:
        marlin_server.print_state = PrintState::Pausing_WaitIdle;
        break;
    case PrintState::Pausing_WaitIdle:
        if ((planner.movesplanned() == 0) && (queue.length == 0) && gcode.busy_state == GcodeSuite::NOT_BUSY) {
            ...
            marlin_server.print_state = PrintState::Pausing_ParkHead;
        }
        break;
    ...
    case PrintState::Resuming_Begin:
        ...
        if (crash_s.is_repeated_crash() && xy_axes_length_ok() != Axis_length_t::ok) {
            ...
            marlin_server.print_state = PrintState::CrashRecovery_Lifting;
            break;
        }
...
#if ENABLED(CRASH_RECOVERY)
        // TODO: the following should be moved to PrintState::Aborting_ParkHead once the "stopping"
        // state is handled properly
        endstops.enable_globally(false);
        crash_s.reset();
#endif // ENABLED(CRASH_RECOVERY)

        marlin_server.print_state = PrintState::Aborting_WaitIdle;
        break;
    case PrintState::Aborting_WaitIdle:
        ...
        marlin_server.print_state = PrintState::Aborting_ParkHead;
        break;
    ...
    case PrintState::Finishing_WaitIdle:
        if ((planner.movesplanned() == 0) && (queue.length == 0)) {
#if ENABLED(CRASH_RECOVERY)
            // TODO: the following should be moved to PrintState::Finishing_ParkHead once the "stopping"
            // state is handled properly
            endstops.enable_globally(false);
            crash_s.reset();
#endif // ENABLED(CRASH_RECOVERY)
            ...
            if (print_job_timer.isRunning())
                print_job_timer.stop();
            marlin_server.print_state = PrintState::Finishing_ParkHead;
        }
        break;
    ...
    case PrintState::Exit:
        marlin_server_finalize_print();
        fsm_destroy(ClientFSM::Printing);
        marlin_server.print_state = PrintState::Idle;
        break;
    ...
    case PrintState::CrashRecovery_Begin: {
        ...
        marlin_server.print_state = PrintState::CrashRecovery_Retracting;
        break;
    }
    ...
    case PrintState::CrashRecovery_Lifting: {
        ...
        marlin_server.print_state = PrintState::CrashRecovery_XY_Measure;
        break;
    }
    case PrintState::CrashRecovery_XY_Measure: {
        ...
        marlin_server.print_state = PrintState::CrashRecovery_XY_HOME;
        break;
    }
    case PrintState::CrashRecovery_XY_HOME: {
        ...
        if (alok != Axis_length_t::ok) {
            marlin_server.print_state = PrintState::CrashRecovery_Axis_NOK;
        ...
        marlin_server.print_state = PrintState::CrashRecovery_Repeated_Crash;
        break;
    }
...
void marlin_server_resuming_begin() {
    ...
    if (marlin_server_print_reheat_ready()) {
        ...
        //print_client::set_display_nozzle(marlin_server.resume.nozzle_temp); // TODO what was this for? We should not call client finctions in server
#if (FAN_COUNT > 0)
        thermalManager.set_fan_speed(0, 0); //disable print fan
#endif
        marlin_server.print_state = PrintState::Resuming_Reheating;
    }
}
...
static uint64_t _server_update_vars(uint64_t update) {
    ...
    if (update & MARLIN_VAR_MSK(MARLIN_VAR_PRNSTATE)) {
        PrintState print = marlin_server.print_state;
...
static void mbl_error(int error_code) {
    if (marlin_server.print_state != PrintState::Printing && marlin_server.print_state != PrintState::Pausing_Begin)
        return;

    marlin_server.print_state = PrintState::Pausing_Failed_Code;
