...
// OTP memory start address
    ...
    OTP_DATAMATRIX_BINARY_SIZE = 13,   // compiled size of datamatrix_t - as returned after conversion from text
};

_Static_assert(sizeof(datamatrix_t) == OTP_DATAMATRIX_BINARY_SIZE, "datamatrix size inconsistent");
_Static_assert(sizeof(serial_nr_t) == OTP_DATAMATRIX_TEXT_SIZE + 1, "serial number size inconsistent");
_Static_assert(sizeof(MAC_addr) == OTP_MAC_ADDRESS_SIZE, "MAC addr size inconsistent");
_Static_assert(sizeof(board_revision_t) == 2, "board_revision_t size inconsistent");
_Static_assert(sizeof(OTP_v0) == 32, "OTP_v0 size inconsistent");
_Static_assert(sizeof(OTP_v3) == 76, "OTP_v3 size inconsistent");
_Static_assert(sizeof(OTP_v4) == 38, "OTP_v4 size inconsistent");
_Static_assert(sizeof(OTP_v5) == 32, "OTP_v5 size inconsistent");
...
bool decode_datamatrix(const uint8_t *src, size_t size, datamatrix_t *dm) {
    ...
    // daily serial
    dm->daily_serial_number = scan_digit_group(src, 4);
