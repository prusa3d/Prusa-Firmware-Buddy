...
const AxisConfig_t selftest::Config_XAxis = { .partname = "X-Axis", .length = X_BED_SIZE, .fr_table_fw = XYfr_table, .fr_table_bw = XYfr_table, .length_min = X_BED_SIZE, .length_max = X_BED_SIZE + X_END_GAP, .axis = X_AXIS, .steps = xy_fr_table_size * 2, .movement_dir = 1 }; // MINI has movement_dir -1

const AxisConfig_t selftest::Config_YAxis = { .partname = "Y-Axis", .length = Y_BED_SIZE, .fr_table_fw = XYfr_table, .fr_table_bw = XYfr_table, .length_min = Y_BED_SIZE, .length_max = Y_BED_SIZE + Y_END_GAP, .axis = Y_AXIS, .steps = xy_fr_table_size * 2, .movement_dir = 1 };
...
static constexpr HeaterConfig_t Config_HeaterBed = { .partname = "Bed", .getTemp = []() { return thermalManager.temp_bed.celsius; }, .setTargetTemp = [](int target_temp) { thermalManager.setTargetBed(target_temp); }, .refKp = Temperature::temp_bed.pid.Kp, .refKi = Temperature::temp_bed.pid.Ki, .refKd = Temperature::temp_bed.pid.Kd, .heatbreak_fan = fanCtlHeatBreak[0], .print_fan = fanCtlPrint[0], .heat_time_ms = 60000, .start_temp = 40, .undercool_temp = 39, .target_temp = 110, .heat_min_temp = 50, .heat_max_temp = 65, .heatbreak_min_temp = -1, .heatbreak_max_temp = -1, .heater_load_stable_ms = 200,
...
static constexpr LoadcellConfig_t Config_Loadcell[] = { { .partname = "Loadcell", .heatbreak_fan = fanCtlHeatBreak[0], .print_fan = fanCtlPrint[0], .cool_temp = 50, .countdown_sec = 5, .countdown_load_error_value = 40, .tap_min_load_ok = 80, .tap_max_load_ok = 500, .tap_timeout_ms = 2000, .z_extra_pos = 100, .z_extra_pos_fr = uint32_t(maxFeedrates[Z_AXIS]), .max_validation_time = 1000 } };
