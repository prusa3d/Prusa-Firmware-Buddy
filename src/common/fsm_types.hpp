...
protected:
    ...
    bool prior_command_in_queue0 = false; // when inserting create to queue1, last inserted create in queue0 has priority
...
public:
    ...
    bool TryPushCreate(ClientFSM type, BaseData data);
...
protected:
    bool pushCreate(SmartQueue *pQueues, size_t sz, ClientFSM type, BaseData data, const char *fnc, const char *file, int line);
...
public:
    bool PushCreate(ClientFSM type, BaseData data, const char *fnc, const char *file, int line) {
    ...
    bool PushDestroyAndCreate(ClientFSM old_type, ClientFSM new_type, BaseData data, const char *fnc, const char *file, int line) {
        if (!pushDestroy(queues, SZ, old_type, fnc, file, line))
            return false;
        return pushCreate(queues, SZ, new_type, data, fnc, file, line);
    }
