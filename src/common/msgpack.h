...
private:
    ...
    void pack_type(const T &value) {
        if constexpr (is_container<T>::value || is_stdarray<T>::value) {
            pack_array(value);
        } else {
...
inline void Packer::pack_type(const float &value) {
    ...
    if (fractional_remainder == 0) { // Just pack as int
        ...
        float implied_mantissa = fabs(static_cast<double>(full_mantissa)) - static_cast<double>(1.0f);
...
inline void Packer::pack_type(const double &value) {
    ...
    if (fractional_remainder == 0) { // Just pack as int
        ...
        double implied_mantissa = fabs(full_mantissa) - static_cast<double>(1.0f);
