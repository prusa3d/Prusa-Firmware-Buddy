...
enum eevar_id {
    // basic variables
    EEVAR_VERSION = 0x00,                     // uint16_t eeprom version - Not in config store
    EEVAR_FEATURES = 0x01,                    // uint16_t feature mask - Not in config store
    EEVAR_DATASIZE = 0x02,                    // uint16_t eeprom data size  - Not in config store
    EEVAR_FW_VERSION = 0x03,                  // uint16_t encoded firmware version (e.g. 403 for 4.0.3)  - Not in config store
    EEVAR_FW_BUILD = 0x04,                    // uint16_t firmware build number  - Not in config store
    EEVAR_FILAMENT_TYPE = 0x05,               // uint8_t  filament type in the first extruder
    EEVAR_FILAMENT_COLOR = 0x06,              // uint32_t filament color (rgb) - Not in config store
    ...
    EEVAR_ZOFFSET_DO_NOT_USE_DIRECTLY = 0x0b, // float zoffset - Not in config store
...
// lan variables
    ...
    EEVAR_SELFTEST_RESULT_V1 = 0x28, //!< Not used, was EEVAR_SELFTEST_RESULT, replaced by EEVAR_SELFTEST_RESULT_V2 - Not in config store
