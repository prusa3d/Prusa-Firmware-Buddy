    ...
    } else if (msg.type == VARIANT8_USER) // event received
    {
        ...
        case MARLIN_EVT_DialogOpen:
            if (client->dialog_open_cb)
                client->dialog_open_cb((uint8_t)msg.ui32, (uint8_t)(msg.ui32 >> 8));
            break;
        case MARLIN_EVT_DialogClose:
            if (client->dialog_close_cb)
                client->dialog_close_cb((uint8_t)msg.ui32);
            break;
        case MARLIN_EVT_DialogChange:
            if (client->dialog_change_cb)
                client->dialog_change_cb((uint8_t)msg.ui32, (uint8_t)(msg.ui32 >> 8), (uint8_t)(msg.ui32 >> 16), (uint8_t)(msg.ui32 >> 24));
            break;
            //not handled events
