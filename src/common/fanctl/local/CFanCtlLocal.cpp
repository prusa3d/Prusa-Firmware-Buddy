...
//------------------------------------------------------------------------------
// CFanCtlLocal implementation

CFanCtlLocal::CFanCtlLocal(const OutputPin &pinOut, const InputPin &pinTach,
...
uint16_t CFanCtlLocal::scalePWM(uint8_t pwm) const {
    return static_cast<uint16_t>(pwm) * m_pwm.get_max_PWM() / 255;
}

uint16_t CFanCtlLocal::unscalePWM(uint8_t pwm) const {
    return static_cast<uint16_t>(pwm) * 255 / m_pwm.get_max_PWM();
}

bool CFanCtlLocal::setPWM(uint8_t pwm) {
    if (selftest_mode)
        return false;
...
bool CFanCtlLocal::SelftestSetPWM(uint8_t pwm) {
    if (!selftest_mode)
        return false;
    m_PWMValue = scalePWM(pwm);
    return true;
...
bool CFanCtlLocal::setPhaseShiftMode(uint8_t psm) {
    if (selftest_mode)
        return false;
...
void CFanCtlLocal::safeState() {
    m_pwm.safeState();
    selftest_mode = false;
}

bool CFanCtlLocal::getRPMIsOk() {
    if (m_PWMValue && (getActualRPM() < m_MinRPM))
        return false;
    return true;
}

void CFanCtlLocal::EnterSelftestMode() {
    if (selftest_mode)
        return;
    selftest_mode = true;
...
void CFanCtlLocal::ExitSelftestMode() {
    if (!selftest_mode)
        return;
    selftest_mode = false;
